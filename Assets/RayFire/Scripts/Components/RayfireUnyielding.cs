            ...
            for (int i = 0; i < uny.connHost.rigidList.Count; i++)
                if (uny.connHost.rigidList[i].physics.meshCollider != null)
                    if (collidersHash.Contains (uny.connHost.rigidList[i].physics.meshCollider) == true)
                    {
                        ...
                        {
                            uny.connHost.rigidList[i].simulationType = (SimType)uny.simulationType;
                            RFPhysic.SetSimulationType (uny.connHost.rigidList[i].physics.rigidBody, uny.connHost.rigidList[i].simulationType, 
                                ObjectType.Mesh, uny.connHost.rigidList[i].physics.useGravity, uny.connHost.rigidList[i].physics.solverIterations);
                        }
                    }
        }
                ...
                if (rigidRoot.cluster.shards[i].col != null)
                    if (collidersHash.Contains (rigidRoot.cluster.shards[i].col) == true)
                    {
                        ...
                        if (simulationType != UnySimType.Original)
                            rigidRoot.cluster.shards[i].sm = (SimType)simulationType;
                    }
        }
